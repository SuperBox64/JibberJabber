...
class SwiftTranspiler:
    ...
    def transpile(self, program: Program) -> str:
        ...
        if self._needs_log_import(program.statements) and T.get('logImport'):
            lines.append(T['logImport'])
    ...
    def _needs_log_import(self, stmts) -> bool:
        for s in stmts:
            if isinstance(s, LogStmt):
                return True
            if isinstance(s, IfStmt):
                if self._needs_log_import(s.then_body):
                    return True
                if s.else_body and self._needs_log_import(s.else_body):
                    return True
            if isinstance(s, LoopStmt) and self._needs_log_import(s.body):
                return True
            if isinstance(s, FuncDef) and self._needs_log_import(s.body):
                return True
        return False
