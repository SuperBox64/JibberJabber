...
class JavaScriptTranspiler:
    ...
    def expr(self, node: ASTNode) -> str:
        ...
        if isinstance(node, Literal):
            ...
            if node.method == 'upper': return f'{s}.toUpperCase()'
            if node.method == 'lower': return f'{s}.toLowerCase()'
            if node.method == 'length': return f'{s}.length'
            if node.method == 'trim': return f'{s}.trim()'
            if node.method == 'contains' and len(node.args) >= 2: return f'{s}.includes({self.expr(node.args[1])})'
            if node.method == 'replace' and len(node.args) >= 3: return f'{s}.replaceAll({self.expr(node.args[1])}, {self.expr(node.args[2])})'
            if node.method == 'split' and len(node.args) >= 2: return f'{s}.split({self.expr(node.args[1])})'
            if node.method == 'substring' and len(node.args) >= 3: return f'{s}.substring({self.expr(node.args[1])}, {self.expr(node.args[2])})'
        return ""
